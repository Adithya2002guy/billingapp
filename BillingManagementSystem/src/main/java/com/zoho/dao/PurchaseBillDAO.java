...
public class PurchaseBillDAO {
    ...
    public PurchaseBillDAO(){
    ...
    }public PurchaseBill getPurchaseBillById(int billId){
        ...
        try{
            ...
            String query = "SELECT bill_id, supplier_name, date FROM purchasebill WHERE bill_id = ?";
            ...
            if(resultSet.next()){
                ...
                purchaseBill = new PurchaseBill(billId, supplierName, date, total);
            }
    ...
    public List<Map<String, Object>> getPurchaseLineItems(int billId) {
        ...
        try {
            connection = dbUtil.getConnection();
            String query = "SELECT pi.product_id, pi.quantity, pi.total, p.name, p.price " +
            ...
            while(resultSet.next()) {
                ...
                item.put("productName", resultSet.getString("name"));
                item.put("quantity", resultSet.getInt("quantity"));
                item.put("price", resultSet.getDouble("price"));
                item.put("total", resultSet.getDouble("total"));
                lineItems.add(item);
            }
        } catch(SQLException e) {
            System.err.println("Error fetching sales line items: " + e.getMessage());
        } finally {
            // Close resources
            try {
                if(resultSet != null) resultSet.close();
